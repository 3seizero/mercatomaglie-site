...
export async function stampaQr(voci) {
  ...
  for (const v of voci) {
    ...
    cards.push(`<div class="card"><div class="head"><div class="brand">MERCATI DI MAGLIE</div><div class="sub">Area Mercatale · presenza espositore</div></div>
      <img src="${png}"><div class="nome">${esc(v.nome)}</div><div class="sotto">${esc(v.sotto || "")}</div><div class="tok">${esc(v.token)}</div></div>`);
  }
  const html = `<!doctype html><html lang="it"><head><meta charset="utf-8"><title>QR espositori</title><style>
    ...
    .brand{font-weight:800;letter-spacing:2px;font-size:11pt} .sub{font-size:8pt;color:#9a8070;letter-spacing:1px;text-transform:uppercase}
